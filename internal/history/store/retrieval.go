...
func (s *Store) RawCandidates(sessionID, snapshotID string) ([]RawCandidate, error) {
	...
	statement := `SELECT kind,source_id,snapshot_id,source_path,archive,relative_path,vault_version,content_sha256,size,mod_time_unix FROM (
		SELECT l.kind,sh.public_id AS source_id,NULL AS snapshot_id,l.source_path,l.archive,l.relative_path,l.vault_version,
			sh.current_sha256 AS content_sha256,sh.size,sh.mtime_unix AS mod_time_unix,CASE l.kind WHEN 'provider_live' THEN 0 ELSE 1 END AS preference,
	...
	statement += ` UNION ALL
		SELECT l.kind,NULL,ps.public_id,l.source_path,l.archive,l.relative_path,l.vault_version,
			ps.content_sha256,ps.size,0 AS mod_time_unix,2 AS preference,ps.created_at AS recency
	...
	for rows.Next() {
		...
		if err := rows.Scan(&value.Kind, &source, &snapshot, &value.SourcePath, &value.Archive, &value.RelativePath, &value.VaultVersion, &value.ContentSHA256, &value.Size, &value.ModTimeUnix); err != nil {
