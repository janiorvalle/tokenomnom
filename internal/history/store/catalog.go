...
var catalogSelect = `SELECT s.public_id,s.provider,s.native_session_id,s.first_ts,s.last_ts,s.cwd,s.repository_name,s.branch,
	...
	(SELECT COUNT(*) FROM prompts p WHERE p.session_id=s.id AND EXISTS(SELECT 1 FROM occurrences o JOIN locations l ON l.id=o.location_id WHERE o.prompt_id=p.id AND l.available=1)),
	(SELECT COUNT(*) FROM occurrences o JOIN prompts p ON p.id=o.prompt_id JOIN locations l ON l.id=o.location_id WHERE p.session_id=s.id AND l.available=1),
